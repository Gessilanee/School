class  Admin::DisciplinesController < Admin::BaseController
  ...
  def create
    ...
    if @discipline.save
      flash[:notice] = 'Disciplina criada com sucesso.'
      redirect_to  admin_disciplines_path
    else
      flash[:alert] = 'Erro ao cadastrar disciplinas, por favor corrigir!.'
      render :new
    end
  end
  ...
  def update
    if @discipline.update(discipline_params)
      ...
      flash[:alert] = 'Erro ao atualizar disciplina, por favor corrigi.'
      render :edit
    end
  end 
  ...
  def destroy
    @discipline.destroy
    flash[:notice] = 'Disciplina excluída com sucesso.'
